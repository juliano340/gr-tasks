...
export default function TaskList({ tasks, plan }: { tasks: Task[], plan: string }) {
    ...
    <>
        ...
        {optimisticTasks.map((task, index) => (
          ...
          >
            <div className="flex items-center gap-3.5">
              ...
              >
                <div className={`w-5.5 h-5.5 rounded-md border-2 flex items-center justify-center transition-all duration-300 ${
                  task.completed
              ...
              <div className="flex-1 min-w-0">
                <h3 className={`text-sm font-semibold leading-tight transition-all duration-300 ${
                ...
                {!task.completed && (
                  <>
                    {task.priority === "urgent" ? (
                      <span className="text-[10px] font-bold text-amber-300 bg-amber-400/20 px-2 py-0.5 rounded-full">Urgente</span>
                    ) : task.priority === "normal" ? (
                      <span className="text-[10px] font-bold text-indigo-200 bg-white/10 px-2 py-0.5 rounded-full">Normal</span>
                    ) : (
                      <span className="text-[10px] font-bold text-white/40 bg-white/5 px-2 py-0.5 rounded-full">Baixa</span>
                    )}
                  </>
                )}
                {task.completed && (
                  <span className="text-[10px] font-bold text-emerald-300 bg-emerald-400/20 px-2 py-0.5 rounded-full">Feito</span>
                )}
      ...
      {taskToDelete && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/50 backdrop-blur-sm" onClick={() => setTaskToDelete(null)} />
            ...
            <div className="p-6">
              <div className="flex items-start gap-4 mb-5">
                <div className="w-11 h-11 bg-red-50 rounded-xl flex items-center justify-center flex-shrink-0">
                  <svg className="w-5.5 h-5.5 text-red-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                  </svg>
                </div>
                <div>
                  <h3 className="text-base font-extrabold text-gray-900 tracking-tight">Deletar tarefa?</h3>
              ...
              <div className="bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 mb-6 flex items-center gap-3">
                <div className={`w-2 h-2 rounded-full flex-shrink-0 ${
                  taskToDelete.priority === "urgent" ? "bg-amber-400" : taskToDelete.priority === "normal" ? "bg-indigo-500" : "bg-gray-400"
                }`} />
                <div className="min-w-0">
                  <p className="text-sm font-bold text-gray-800 truncate">{taskToDelete.title}</p>
                </div>
              </div>
              <div className="flex gap-3">
                <button onClick={() => setTaskToDelete(null)} className="flex-1 py-2.5 px-4 bg-gray-100 text-gray-600 text-sm font-bold rounded-xl hover:bg-gray-200 transition-all duration-200">Cancelar</button>
                <button onClick={confirmDelete} className="flex-1 py-2.5 px-4 bg-red-500 text-white text-sm font-bold rounded-xl shadow-md shadow-red-200 hover:bg-red-600 hover:shadow-lg hover:shadow-red-200 hover:-translate-y-0.5 transition-all duration-200 flex items-center justify-center gap-2">Deletar</button>
              </div>
            </div>
          </div>
        </div>
